...
class ArchitectureCLI:
    ...
    def _chat_mode(self):
        ...
        console.print(
            "[dim]Type 'done' to finish or 'back' to return to the main menu.[/dim]\n")
        ...
        while True:
            ...
            if not user_input:
                console.print(
                    "[dim]Please enter a response or type 'done' to finish.[/dim]")
                continue
            ...
            if conversation_round % 5 == 0:
                console.print(
                    "[dim]Auto-summarizing recent discussion...[/dim]")
                ...
                console.print(
                    f"[italic cyan]{partial_summary}[/italic cyan]\n")

        # Merge summaries for better requirement extraction
        if summarized_points:
            self.chat_history.append(
                "AI Summary Blocks: " + " ".join(summarized_points))
        ...
        if Confirm.ask("\nAdd these requirements?"):
            ...
            for line in lines:
                if line.strip().startswith('REQ'):
                    parts = line.split('|')
                    if len(parts) >= 1:
                        req_text = parts[0].split(
                            ':', 1)[1].strip() if ':' in parts[0] else parts[0]
                        priority = parts[1].split(
                            ':')[1].strip() if len(parts) > 1 else None
                        category = parts[2].split(
                            ':')[1].strip() if len(parts) > 2 else None
            ...
            console.print(
                f"[green]✓[/green] Added {len(self.requirements)} requirements")
    ...
    def _upload_documents(self):
        ...
        while True:
            ...
            try:
                ...
                console.print(f"[green]✓[/green] Processed: {doc.filename}")
                console.print(f"  Type: {doc.document_type}")
                console.print(f"  Size: {len(doc.content):,} characters")

                # Show content preview
                if Confirm.ask("\n  Preview content that will be sent to AI?", default=False):
                    preview_length = 500
                    ...
                    if len(doc.content) > preview_length:
                        if Confirm.ask("\n  View full content?", default=False):
                            console.print("\n[bold]Full Content:[/bold]")
                            console.print(
                                Panel(doc.content, border_style="green"))
    ...
    def _review_requirements(self):
        ...
        if self.supporting_docs:
            ...
            doc_table = Table(title="Uploaded Documents")
            doc_table.add_column("№", style="cyan")
            doc_table.add_column("Filename", style="white")
            doc_table.add_column("Type", style="yellow")
            doc_table.add_column("Size", style="green")

            for i, doc in enumerate(self.supporting_docs, 1):
                doc_table.add_row(
                    str(i),
                    doc.filename,
            ...
            # Option to view document content
            if Confirm.ask("\nView content of any document?", default=False):
                doc_num = Prompt.ask(
                    "Enter document number",
                    choices=[str(i) for i in range(
                        1, len(self.supporting_docs) + 1)]
                )
                selected_doc = self.supporting_docs[int(doc_num) - 1]

                console.print(
                    f"\n[bold]Document: {selected_doc.filename}[/bold]")
                console.print(f"Type: {selected_doc.document_type}")
                console.print(
                    f"Size: {len(selected_doc.content):,} characters\n")
                console.print(
                    "[bold cyan]Content that will be sent to AI:[/bold cyan]")
                console.print(Panel(selected_doc.content,
    ...
    def _generate_architecture(self):
        ...
        if not self.requirements:
            console.print(
                "[red]✗[/red] No requirements found. Add requirements first.")
            return
        ...
        with console.status("[bold green]Thinking..."):
            chat_history_text = "\n".join(
                self.chat_history) if self.chat_history else None
        ...
        if Confirm.ask("\nSave architecture to file?"):
            output_file = Prompt.ask(
                "Output filename", default="solution_architecture.md")
